...
import { PrototypeDice } from './PrototypeDice';
import { PrototypeHUD } from './PrototypeHUD';
...
export default function PrototypeGameLayout({ initialPlayers, onMainMenu }: GameLayoutProps) {
  ...
  return (
    <div className={`flex flex-col h-screen overflow-hidden relative transition-colors duration-300 ${isShaking ? 'animate-[shake_0.5s_ease-in-out]' : ''}`} style={{ backgroundColor: 'var(--color-cream)', color: 'var(--color-navy-dark)' }}>
      ...
      <main className="flex-1 flex flex-col lg:flex-row min-h-0 relative z-0">
        ...
        <aside 
          className={`hidden lg:flex absolute top-0 bottom-0 left-0 z-40 transition-transform duration-300 ${isDesktopLogOpen ? 'translate-x-0' : '-translate-x-full'} w-[25%] xl:w-[28%] p-4 flex-col shadow-[4px_0_24px_rgba(30,58,95,0.08)] border-r backdrop-blur-md bg-white/60`}
        ...
        <aside 
          className="hidden lg:flex lg:w-[25%] xl:w-[28%] shrink-0 border-l p-6 flex-col gap-6 overflow-y-auto shadow-[-4px_0_24px_rgba(30,58,95,0.08)] z-10 transition-colors duration-300 backdrop-blur-md bg-white/60"
          style={{ borderColor: 'rgba(201,168,76,0.3)' }}
        >
          <PrototypeHUD activePlayer={activePlayer} players={gameState.players} layout="desktop" />
          <div 
        ...
        <div 
          className="lg:hidden h-[40%] w-full border-t p-4 pb-safe flex flex-col gap-4 items-center justify-center shadow-[0_-8px_24px_rgba(30,58,95,0.08)] z-20 overflow-y-auto relative transition-colors duration-300 backdrop-blur-md bg-white/80"
          style={{ borderColor: 'rgba(201,168,76,0.3)' }}
        >
          <div className="w-full">
            <PrototypeHUD activePlayer={activePlayer} players={gameState.players} layout="mobile" />
          </div>
          <div 
