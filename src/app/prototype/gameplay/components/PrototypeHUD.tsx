...
export const PrototypeHUD: React.FC<HUDProps> = ({ activePlayer, players = [], gameStatus = 'playing', layout = 'desktop' }) => {
      ...
      <div className="flex gap-1 ml-2">
        {player.activeEffects.map((effect, idx) => (
          <span key={idx} className="relative text-[10px] font-bold bg-[var(--color-cream-dark)]/40 text-[var(--color-navy-dark)] border border-[var(--color-gold)]/20 rounded-full px-1.5 py-0.5" title={effect.type}>
            ...
            {effect.duration > 1 && (
              <span className="absolute -bottom-1 -right-1 bg-red-500 text-white text-[7px] leading-none rounded-full w-3 h-3 flex items-center justify-center font-bold shadow-sm border border-white">
                {effect.duration}
              </span>
            )}
          </span>
  ...
  const jakartaHUDContainerClass = isJakarta 
    ...
    : "bg-white/90 rounded-xl shadow-[0_4px_20px_rgba(30,58,95,0.08)] border-2 overflow-hidden flex flex-col transition-all duration-300 shrink-0";
